...
void testApp::setup(){
    ...
    gui.add(fftToggle.setup("FFT bin magnitudes (pitch/timbre/volume) (512 inputs)", false));
    gui.add(mfccToggle.setup("MFCCs (timbre/vocal) (13 inputs)", false));
    gui.add(chromagramToggle.setup("Const-Q analyser (12 bands/oct) (104 inputs)", false));
    gui.add(peakFrequencyToggle.setup("Peak frequency (pitch) (1 input)", false));
    gui.add(centroidToggle.setup("Spectral centroid (timbre) (1 input)", false));
    gui.add(rmsToggle.setup("RMS (volume) (1 input)", false));
	...
	myFont5.loadFont("arial.ttf", 7); //requires this to be in bin/data/
...
void testApp::draw(){
    ...
    for(int i=0; i < oct.nAverages; i++) {
		ofSetColor(0, int(255/nAverages)*int(i/8), 255- int(255 / nAverages));
